...
def list_all_file_path(path):
    """
    this function aim to list all of file of this path your give to and return a list that contain the deepest file
    path.

    :param path: folder path
    :return: a list that contain the deepest file path, eg: ['test/test.file', 'test/test_folder/test.file']
    """
    list_path = []
    for root, dirs, files in os.walk(path):
        for _ in files:
            list_path.append(os.path.join(root, _))
    return list_path


def handle_obfuscate(file_path, output_file):
    global conf

    conf = Configuration(file_path=file_path)
    ...
    TokenPrinter(source, ce.result, (output_file if output_file else None))


def main():
    try:
        provide_file = sys.argv[1]

        # 处理单个文件
        if os.path.isfile(provide_file):
            if provide_file.aplit('.')[-1] == 'py':
                output_file = open("{}_obfuscate.py".format(provide_file.split('.')[0]), "w+")
                handle_obfuscate(file_path=provide_file, output_file=output_file)
            else:
                print '只能处理 python 文件'
                sys.exit(1)

        # 处理整个文件夹
        elif os.path.isdir(provide_file):
            if not os.path.exists('obfuscate_output'):
                os.mkdir('obfuscate_output')

            list_path = list_all_file_path(provide_file)

            # 创建相同的目录结构
            for i in list_path:
                folder_path = os.path.join('obfuscate_output', "/".join(i.split('/')[:-1]))
                if not os.path.exists(folder_path):
                    os.makedirs(folder_path)

            # 遍历处理迭代文件夹
            for i in list_path:
                # 只处理 python 文件, 其他一律跳过
                if i.split('.')[-1] == 'py':
                    output_file = os.path.join('obfuscate_output', i)
                    open_file = open(output_file, "w+")
                    handle_obfuscate(i, open_file)

                # 如果不是 python 文件,就只复制过去
                else:
                    import shutil
                    output_file = os.path.join('obfuscate_output', i)
                    shutil.copy(i, output_file)

        else:
            raise getopt.GetoptError("输入的什么玩意?")
